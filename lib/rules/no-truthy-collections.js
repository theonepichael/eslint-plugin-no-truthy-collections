 ...
 * @note TypeScript-based detection only works if TypeScript is installed and parser services are available.
 */

// ---- Naming Heuristics Patterns (used if strictNaming is enabled) ----
// These are used to heuristically detect array-like and object-like variable names.
const ARRAY_NAMING_PATTERNS = [
  /[Aa]rray$/,
  /[Ll]ist$/,
  /[Ii]tems$/,
  /[Ee]lements$/,
...
export default {
  meta: {
    ...
    docs: {
      description:
        'Prevent arrays and objects from being used in boolean contexts. TypeScript-based detection only works if TypeScript is installed and parser services are available.',
      category: 'Possible Errors',
      recommended: true,
      url: 'https://github.com/theonepichael/eslint-plugin-no-truthy-collections/blob/main/README.md',
    },
    fixable: 'code',
    hasSuggestions: true,
    schema: [
  ...
  create(context) {
    ...
    function analyzeNaming(node) {
      ...
      // Array-like patterns
      for (const pattern of ARRAY_NAMING_PATTERNS) {
      ...
      for (const pattern of OBJECT_NAMING_PATTERNS) {
        if (pattern.test(name)) {
    ...
    function reportIssue(node, analysis, messageContext = 'boolean') {
      ...
      if (
        ...
        if (collectionType === 'arraylike') {
          report.suggest = [
            {
              desc: `Safe default: Use ${suggestion} to check for items`,
              fix(fixer) {
                return fixer.replaceText(node, suggestion);
              },
            },
